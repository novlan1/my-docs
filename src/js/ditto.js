...
function init_sidebar_section() {
  ...
  const searchReg = searchStr.match(/\#(.*)\//);

  let type = "es6";

  if (searchReg && searchReg[1]) type = searchReg[1];

  const sideBarFile = `${ditto["sidebar_file_prefix"]}-${type}.md`;

  $.get(
    sideBarFile,
    function (data) {
      const extraLink = `<div class='shame-link'><a href='https://uwayfly.com'>杨国旺的博客</a></div>`
      $(ditto.sidebar_id).html(extraLink + marked(data));
      console.log(extraLink + marked(data))
...
function searchbar_listener(event) {
  ...
  if (q !== "") {
    var url =
      "https://github.com/ruanyf/es6tutorial/search?utf8=✓&q=" +
...
function create_banner(element) {
  ...
  var banner = $(
    '<a href="https://datayi.cn/w/a9BLv2LP" style="color: #333333;" target="_blank"><div style="' +
      styleStr +
      '">' +
      text +
      "</div></a>"
...
function create_page_anchors() {
  ...
  for (var i = 2; i <= 4; i++) {
    ...
    $("#content h" + i).map(function () {
      ...
      var _content = content.replace(/</g, "&lt;");
      _content = _content.replace(/>/g, "&gt;");
      headers.push(_content);
